...
// Define external server to log updates
$SQLSERVER = "acmeautodns.database.windows.net"
$DBNAME = "ACMEAutomation"
$DBUSER = "acmesqladmin"
$DBUSERPW = "secret_password"
...
// Try to connect to the tracking DB
try{
   $db = new PDO ( "sqlsrv:server = tcp:$SQLSERVER,1433; Database = $DBNAME", "$DBUSER", "$DBUSERPW", [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION]);
}catch (\Exception $E) {
...
// run the SQL statement to add the record
$stmt->execute();

// return the record ID for this request
